...
namespace AwesomeCare.API.Controllers
{
    ...
    public class ClientProgramController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([FromBody] PostClientProgram postClientProgram)
        {
            ...
            var newClientProgram = await _clientProgramRepository.InsertEntity(ClientProgram);
            var getClientProgram = Mapper.Map<GetClientProgram>(newClientProgram);
        ...
        /// <returns></returns>
        [HttpPut]
        [ProducesResponseType(type: typeof(GetClientProgram), statusCode: StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Put([FromBody] PutClientProgram model)
        {
            ...
            var entity = await _clientProgramRepository.GetEntity(model.ProgramId);
            var putEntity = Mapper.Map(model, entity);
            var updateEntity = await _clientProgramRepository.UpdateEntity(putEntity);
            var getEntity = Mapper.Map<GetClientProgram>(updateEntity);
            return Ok(getEntity);
